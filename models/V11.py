...
class SamClipCD(nn.Module):
    ...
    def SCFE(self, x, t_embedding):
        ...
        if self.prompt == 'clip':
            ...
            weights_scores = F.softmax(self.param_multi_score, dim=0).view(-1, 1, 1, 1, 1)  # (L, 1, 1, 1, 1)
            # 加权融合
    ...
    def forward(self, x1, x2, targets = None,batched_input = None, multimask_output = False, with_amp = False ):
        ...
        x_fuses = []
        x_fuse = self.interacts(x1_orig, x2_orig)  # fusion method
        ...
        x_fuses[-1] = torch.cat((x_fuses[-1], score_fusion), dim = -1)
