...
from models.decoder.transformer_decoder_v0 import MaskDecoder,TwoWayTransformer,MultiMaskDecoder
...
class MoEConvFusion4Experts(nn.Module):
    def __init__(self, in_channels, hidden_dim=128, use_norm=True):
        super().__init__()
        ...
        self.gate = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, 4)  # 4 experts
        )
    ...
    def forward(self, x):
        ...
        H = int(math.sqrt(fused.shape[1]))

        fused = self.out_proj(fused)
        fused = einops.rearrange(fused, 'b (h w) c -> b c h w', h=H)
        return fused
...
class SamClipCD(nn.Module):
    def __init__(self,
        ...
        self.width = width
        self.clip_image_encoder = CLIPVisionTransformer(input_resolution=configer.get("data","input_size") ,pretrained=clip_model, patch_size= 16, get_embeddings=True)
        ...
        self.register_buffer("mem_queue", torch.randn((self.out_cls, n_pts, width)))
        ...
        self.MoE = False
        if configer.get("network", "params")["feats_fusion"] =='MoE' or configer.get("network", "params")["feats_fusion"] =="SCAM":
            self.embed_feats = MoEConvFusion4Experts(
             width, width)
            self.MoE = True
        else:
            self.embed_feats = nn.Sequential(
                nn.Conv2d(width * 4, width, kernel_size=3, stride=1, padding=1),
        ...
        self.aux_head = nn.Sequential(
            nn.Conv2d(width , self.out_cls, kernel_size=1, stride=1, padding=0)
        )
    ...
    def forward(self, x1, x2, targets = None,batched_input = None, multimask_output = False, with_amp = False ):
        ...
        # coarse_score = einops.rearrange(score_fusion, 'b (h w) c -> b c h w',h = int(math.sqrt(score_fusion.shape[1])))
        # coarse_score = F.interpolate(coarse_score, size= coarse_pred.shape[-2:], mode='bilinear', align_corners=False)
        #
        # score_norm = torch.tanh(coarse_score)
        # coarse_pred = coarse_pred + self.gamma_coarse * score_norm

        pred = self.decoder([dense_embed], coarse_pred, t_embedding)

        return {"pred": pred, "dense_embed": dense_embed, "pred_aux": coarse_pred}
